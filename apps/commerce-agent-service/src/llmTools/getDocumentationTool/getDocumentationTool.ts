...
const getDocumentationToolSchema = z.object({
  input: z.string().describe("The query text used to find similar content in the documentation."),
  limit: z.number().optional().describe("Maximum total number of similar content items to return."),
  crowding: z.number().optional().describe("Maximum number of results returned from a single document type."),
  contentTypes: z.array(z.string()).optional().describe("Array of types to be returned. Allowed values: `apiType`,`apiEndpoint`,`referenceDocs`,`guidedDocs`,`userDocs`."),
  products: z.array(z.string()).optional().describe("Array of product names to filter the search results. Allowed values: ['Composable Commerce', 'Frontend', 'Checkout', 'Connect']."),
});
